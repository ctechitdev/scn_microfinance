...
const search =
  "select guarantee_picture, concat(gender,' ',first_name,' ',last_name) as full_name,age, phone_number,whats_app_number,village_namge,districts_name, province_name ,asset_credit_values,limit_credit_values, CASE WHEN asset_status = 1 THEN 'Active' ELSE 'Inactive' END AS asset_status FROM tbl_asset_credit_customer AS tbl_asset_credit_customer LEFT JOIN tbl_customer ON tbl_asset_credit_customer.customer_id = tbl_customer.customer_id left join tbl_districts on tbl_asset_credit_customer.customer_id=tbl_districts.districts_id left join tbl_province on tbl_asset_credit_customer.customer_id=tbl_province.province_id where first_name=? or last_name =? or  age=? or phone_number=? or whats_app_number=? or village_namge=? or districts_name=? or province_name=? or asset_credit_values=? ";
