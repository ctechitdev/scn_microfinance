...
const create_customer = (request, response) => {
  ...
  jwt.verify(request.token, secretkey, (token_error, rstoken) => {
    if (token_error) {
      ...
      connected.query(queries.get_customerByName, [first_name, last_name], (error, results) => {
        ...
        if (nocustomer) {
          ...
          connected.query(queries.add_customer, [
            profile_picture,
            gender,
            first_name,
            last_name,
...
const get_customerByName = (request, response) => {
  ...
  jwt.verify(request.token, secretkey, (token_error, rstoken) => {
    if (token_error) {
      ...
      connected.query(queries.get_customerName, [first_name], (error, results) => {
        if (error) throw error;
        if (results.length) {
          response.json(results);
        } else {
...
const get_customerByphone = (request, response) => {
  ...
  jwt.verify(request.token, secretkey, (token_error, rstoken) => {
    if (token_error) {
      ...
      connected.query(queries.get_customerByPhone, [phone_number], (error, results) => {
        if (error) throw error;
        if (results.length) {
          response.json(results);
        } else {
...
const update_customer = (request, response) => {
  ...
  jwt.verify(request.token, secretkey, (token_error, rstoken) => {
    if (token_error) {
      ...
      connected.query(queries.get_customerByName, [first_name, last_name], (error, results) => {
        const nocustomerFound = results.length;
        if (nocustomerFound) {
          if (error) throw error;
          response.json({ resultsCode: "ບໍ່ສາມາດແກ້ໄຂ customer ໄດ້" });
        } else {
          connected.query(queries.update_customer, [
...
const delete_customer = (request, response) => {
  ...
  jwt.verify(request.token, secretkey, (token_error, rstoken) => {
    if (token_error) {
      ...
      connected.query(queries.check_loan_request, [customer_id], (error, results) => {
        if (error) throw error;
        if (results.length) {
          // ຖ້າມີບໍ່ໃຫ້ມັນລົບ
